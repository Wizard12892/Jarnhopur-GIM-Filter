...
# Hardcoded location for the Revenant Caves
HARDCODED_LOCATIONS = {
    "REVENANTS": {
        "coords": [[3143,3264,0,10238,10052,0]],
        "name": "Revenant Caves",
        "template": (
            "#define CONST_AREA_REVENANTS0 [3143,3264,0,10238,10052,0] // [[Revenant Caves]]\n"
            "#define CONST_REVENANTS_RULE(_cond) rule ((area:CONST_AREA_REVENANTS0) && (_cond))"
        )
    }
}
...
def run_batch(monster_list_file='monsters.txt', template_path='monster_template.txt', output_dir='output'):
    ...
    if merged_results and output_merged_file:
        merged_file = "Monsters_Wiki_Parse.txt" # Name of the merged output file
...
def parse_coordinates(loc_text):
    ...
    # If no coordinates found, try direct format
    if not coords:
        matches = re.finditer(coord_pattern2, loc_text)
        for match in matches:
            x, y = int(match.group(1)), int(match.group(2))
            coords.append((x, y))
...
def format_area_constants(monster_name, locations_text, buffer_size=wander_range_distance):
    ...
    area_blocks = []
    current_area = 0
    location_groups = []  # Store coordinates grouped by location name
    ...
    for block in loc_blocks:
        ...
        if coords:
            ...
            if location_match:
                ...
                # Process location name to clean up formatting
                location_name = location_text
                
                # Remove reference tags
                location_name = re.sub(r'<ref[^>]*?/>|<ref[^>]*?>.*?</ref>', '', location_name)
                
                # Handle piped links
                location_name = re.sub(r'\[\[(.*?)\|(.*?)\]\]', r'\1', location_name)
                
                # Remove location/dungeon suffixes
                location_name = re.sub(r'\s*\((?:location|dungeon)\)', '', location_name)
